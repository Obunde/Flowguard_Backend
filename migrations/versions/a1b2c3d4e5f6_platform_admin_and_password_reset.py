...
down_revision: Union[str, None] = "344690fada1b"
...
def downgrade() -> None:
    ...
    op.drop_column("user", "must_reset_password", schema="master")

    # Any platform-admin rows must go before tenant_id can be NOT NULL again.
    op.execute("DELETE FROM master.\"user\" WHERE role = 'PLATFORM_ADMIN'")
    op.alter_column(
        "user", "tenant_id",
    ...
    if bind.dialect.name == "postgresql":
        ...
        op.execute(
            ...
            "('ADMIN', 'PLANNER', 'TECHNICIAN', 'VIEWER')"
        )
        op.execute(
            'ALTER TABLE master."user" ALTER COLUMN role TYPE user_role '
